...
case 'GLNXA64'
    ...
    if str2double(ver(1)) <= 8
        ...
        LD = ['LDFLAGS=''-pthread -shared -m64'' LINKEXPORTVER=''-Wl,--version-script,',atroot,'/simulator/element/mexFunctionGLNXA64v9.map'''];
    end
    PLATFORMOPTION = [PLATFORMOPTION, LD,' '];   
case 'PCWIN'
    PLATFORMOPTION = [PLATFORMOPTION,'LDFLAGS=''-pthread -shared -m32 -Wl,--version-script,',atroot,'/simulator/element/mexFunctionGLNW32.map''',' '];  
end
