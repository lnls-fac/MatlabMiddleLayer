...
void FFQuadLinearPass(double *r, double le, double kv, int NumSplits, double *cL, double *cK, double *T1, double *T2, double *R1, double *R2, int num_particles)
{	int c, i;
	double *r6, k, tot_cL, len;
	...
	tot_cL = 0;
	for (i=0; i<NumSplits; ++i) tot_cL += cL[i];
...
{	double *pr1, *pr2, *pt1, *pt2 , *pCL, *pCK, le, kv;   
	int *returnptr,fnum, NumSplits;
	int *NewFieldNumbers;
    switch(mode)
		{	case NO_LOCAL_COPY:	/* Obsolete in AT1.3 et fields by names from MATLAB workspace  */
			...
			case MAKE_LOCAL_COPY: 	/* Find field numbers first
				...
				{						
					...
					fnum = mxGetFieldNumber(ElemData,"NumSplits");
					if(fnum<0) 
					    mexErrMsgTxt("Required field 'NumSplits' was not found in the element data structure"); 
					NewFieldNumbers[2] = fnum;
					NumSplits = (int) mxGetScalar(mxGetFieldByNumber(ElemData,0,NewFieldNumbers[2]));
                    
					fnum = mxGetFieldNumber(ElemData,"cL");
					if(fnum<0) 
					    mexErrMsgTxt("Required field 'cL' was not found in the element data structure"); 
					NewFieldNumbers[3] = fnum;
					pCL = mxGetPr(mxGetFieldByNumber(ElemData,0,fnum));
						
					fnum = mxGetFieldNumber(ElemData,"cK");
					if(fnum<0) 
					    mexErrMsgTxt("Required field 'cK' was not found in the element data structure"); 
					NewFieldNumbers[4] = fnum;
					pCK = mxGetPr(mxGetFieldByNumber(ElemData,0,fnum));
			...
			case	USE_LOCAL_COPY:	/* Get fields from MATLAB using field numbers
				...
				{	le = mxGetScalar(mxGetFieldByNumber(ElemData,0,FieldNumbers[0]));
					...
					NumSplits = mxGetScalar(mxGetFieldByNumber(ElemData,0,FieldNumbers[2]));
					pCL = mxGetPr(mxGetFieldByNumber(ElemData,0,FieldNumbers[3]));
					pCK = mxGetPr(mxGetFieldByNumber(ElemData,0,FieldNumbers[4]));
	...
	FFQuadLinearPass(r_in, le, kv, NumSplits, pCL, pCK, pt1, pt2, pr1, pr2 , num_particles);
	return(returnptr);	
}
...
{	
	...
	tmpmxptr = mxGetField(prhs[0],0,"NumSplits");
	if(tmpmxptr)
	    NumSplits = (int) mxGetScalar(tmpmxptr);
	else
	    mexErrMsgTxt("Required field 'NumSplits' was not found in the element data structure"); 
		
	tmpmxptr = mxGetField(prhs[0],0,"cL");
	if(tmpmxptr)
	    pCL = mxGetPr(tmpmxptr);
	else
	    mexErrMsgTxt("Required field 'cL' was not found in the element data structure"); 
		
	tmpmxptr = mxGetField(prhs[0],0,"cK");
	if(tmpmxptr)
	    pCK = mxGetPr(tmpmxptr);
	else
	    mexErrMsgTxt("Required field 'cK' was not found in the element data structure"); 
	...
	r_in = mxGetPr(plhs[0]);
	FFQuadLinearPass(r_in, le, kv, NumSplits, pCL, pCK, pt1, pt2, pr1, pr2, n);
	}
	else
	{   /* return list of required fields */
		...
		mxSetCell(plhs[0],2,mxCreateString("NumSplits"));
	    mxSetCell(plhs[0],3,mxCreateString("cL"));
		mxSetCell(plhs[0],4,mxCreateString("cK"));
