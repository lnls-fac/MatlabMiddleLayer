%% Version SI.V24.02-S05.01 
%  ========================
%
% goal_tunes = [49.096188917357331, 14.151971558423915];
% goal_chrom = [2.549478494984214, 2.527086095938103];
% tw1 = calctwiss(the_ring);
% all(abs([tw1.mux(end), tw1.muy(end)]/2/pi - goal_tunes) <= 0.0001) && all(abs([tw1.chromx, tw1.chromy] - goal_chrom) <= 0.01)

    
% % ===================
% % SI.V24.01 strengths
% % ===================
% 
%     
% % QUADRUPOLES
% % ===========
% QDA_strength  = -1.622404771059;
% QFA_strength  = +3.574849741767;
% QDB1_strength = -2.008744908636;
% QDB2_strength = -3.403732690455;
% QFB_strength  = +4.112048429299;
% QDP1_strength = -2.008744908636;
% QDP2_strength = -3.403732690455;
% QFP_strength  = +4.112048429299;
% 
% Q1_strength   = +2.82467581344;
% Q2_strength   = +4.33553230561;
% Q3_strength   = +3.226244592296;
% Q4_strength   = +3.941477483915;
% 
% % SEXTUPOLES
% % ==========
% SDA0_strength = -80.8337;
% SDB0_strength = -64.9422;
% SDP0_strength = -64.9422;
% SFA0_strength = +52.5696;
% SFB0_strength = +73.7401;
% SFP0_strength = +73.7401;
% 
% SDA1_strength = -162.86481428205505;
% SDA2_strength =  -88.80496224467488;
% SDA3_strength = -139.88675100750922;
% SFA1_strength = +191.66622097823435;
% SFA2_strength = +150.64804173247722;
% SDB1_strength = -141.39698214193538;
% SDB2_strength = -122.12536071422329;
% SDB3_strength = -173.68733738100065;
% SFB1_strength = +227.51676106242556;
% SFB2_strength = +197.58343817406129;
% SDP1_strength = -142.18842705218736;
% SDP2_strength = -122.20521709826868;
% SDP3_strength = -174.10041688365578;
% SFP1_strength = +229.06017410501138;
% SFP2_strength = +198.36906930890160;


% % ========================================================
% % SI.V24.02 strengths
% % Based of average B1 and B2 measurement-calibrated models
% % ========================================================
% 
%     
% % QUADRUPOLES
% % ===========
% Q1_strength   = +2.818385349790;
% Q2_strength   = +4.340291521605;
% Q3_strength   = +3.218059095552;
% Q4_strength   = +3.950643530397;
% 
% QDA_strength  = -1.620412680782;
% QFA_strength  = +3.573153825214;
% QDB1_strength = -2.006565412010;
% QDB2_strength = -3.421252073262;
% QFB_strength  = +4.115192695361;
% QDP1_strength = -2.006565412010;
% QDP2_strength = -3.421252073262;
% QFP_strength  = +4.115192695361;   
% 
% 
% % SEXTUPOLES
% % ==========
% SDA0_strength = -80.8337;
% SDB0_strength = -64.9422;
% SDP0_strength = -64.9422;
% SFA0_strength = +52.5696;
% SFB0_strength = +73.7401;
% SFP0_strength = +73.7401;
% 
% SDA1_strength = -163.01260539788831;
% SDA2_strength = -88.88451042589969;
% SDA3_strength = -139.94644097961901;
% SFA1_strength = +191.79026211252767;
% SFA2_strength = +150.75909139196213;
% SDB1_strength = -141.70051673110981;
% SDB2_strength = -122.31693950143621;
% SDB3_strength = -173.84217627660951;
% SFB1_strength = +227.78411049630401;
% SFB2_strength = +197.77415624706609;
% SDP1_strength = -142.32103643823632;
% SDP2_strength = -122.28713872549834;
% SDP3_strength = -174.17968368905983;
% SFP1_strength = +229.19846477246560;
% SFP2_strength = +198.46694176515152;


% ============================================================
% SI.V24.03 strengths
% Based of average B1, B2 and BC measurement-calibrated models
% ============================================================
    
% QUADRUPOLES
% ===========
Q1_strength   = +2.818370601288;
Q2_strength   = +4.340329381668;
Q3_strength   = +3.218430939674;
Q4_strength   = +3.950686823494;

QDA_strength  = -1.619529595236;
QFA_strength  = +3.573170639946;
QDB1_strength = -2.006761160751;
QDB2_strength = -3.420551898839;
QFB_strength  = +4.115074652461;
QDP1_strength = -2.006761160751;
QDP2_strength = -3.420551898839;
QFP_strength  = +4.115074652461;

% SEXTUPOLES
% ==========
SDA0_strength = -80.8337;
SDB0_strength = -64.9422;
SDP0_strength = -64.9422;
SFA0_strength = +52.5696;
SFB0_strength = +73.7401;
SFP0_strength = +73.7401;

SDA1_strength = -163.00673496033022;
SDA2_strength = -88.88283372136254;
SDA3_strength = -139.94196759537465;
SFA1_strength = +191.76918761338641;
SFA2_strength = +150.74751943200232;
SDB1_strength = -141.68731012406636;
SDB2_strength = -122.31611630101850;
SDB3_strength = -173.83532728484047;
SFB1_strength = +227.74260050056122;
SFB2_strength = +197.75140194181881;
SDP1_strength = -142.31458860004548;
SDP2_strength = -122.28494860530658;
SDP3_strength = -174.17505598949975;
SFP1_strength = +229.17864087362835;
SFP2_strength = +198.45436904861020;
