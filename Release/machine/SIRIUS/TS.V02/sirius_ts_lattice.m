...
% --- drift spaces ---
ldif     = 0.1442; 
l13      = drift('l15', 0.15, 'DriftPass');
l14      = drift('l15', 0.15, 'DriftPass');
...
la2p     = drift('la2p', 0.25777, 'DriftPass');
la3p     = drift('la3p', 0.16-ldif, 'DriftPass');
lb1p     = drift('lb1p', 0.22-ldif, 'DriftPass');
lb3p     = drift('lb3p', 0.25883, 'DriftPass');
l25p     = drift('l25p', 0.25-ldif, 'DriftPass');
lc1p     = drift('lc1p', 0.34-ldif, 'DriftPass');
lc2p     = drift('lc2p', 0.22451, 'DriftPass');
ld1p     = drift('ld1p', 0.33-ldif, 'DriftPass');
ld2p     = drift('ld2p', 0.16407, 'DriftPass');
...
la2   = [la2p, repmat(l20,1,12), bpm, l20, ch, l25, cv, l20];
la3   = [l15, la3p];
lb1   = [l20, l20, lb1p];
lb2   = [l20, l20, l14];
lb3   = [lb3p, repmat(l20,1,18), bpm, l20, ch, l25, cv, l25p];
lc1   = [lc1p, repmat(l20,1,8)];
lc2   = [lc2p, bpm, l20, ch, l25, cv, l25p];
ld1   = [ld1p, repmat(l20,1,5)];
ld2   = [ld2p, l20, l20, bpm, l20, cv, l25, ch, l20];
ld3   = [l22];
ld4   = [l13, l14, repmat(l20,1,7), bpm, l20, cv, l20];
lsep  = [l17, l17];
...
line5 = [ch, septin, lsep, septin, lsep, septin, bpm];
