...
the_ring = the_ring0;
% the_ring = models_from_measurements_dipoles_quadoffset(the_ring);
...
function the_ring = models_from_measurements_dipoles_quadoffset(the_ring0)

% b1_factor = (1 - 0.46/100);  % based on comparison with wiki specs
% b2_factor = (1 - 0.46/100);  % based on comparison with wiki specs
b1_factor = (1 - 0.32/100);  % based on comparison with nominal models
b2_factor = (1 - 0.32/100);  % based on comparison with nominal models
the_ring = the_ring0;
% rescale B1 quadrupoles
index = findcells(the_ring, 'FamName', 'B1');
K = getcellstruct(the_ring, 'PolynomB', index, 1, 2);
the_ring = setcellstruct(the_ring, 'PolynomB', index, b1_factor * K, 1, 2);
% rescale B2 quadrupoles
index = findcells(the_ring, 'FamName', 'B2');
K = getcellstruct(the_ring, 'PolynomB', index, 1, 2);
the_ring = setcellstruct(the_ring, 'PolynomB', index, b2_factor * K, 1, 2);
...
% goal tunes and chroms, as calculated with calctwiss, not atsummary!
