...
% --- correctors ---
corr_len = 0.082;
chv   = sextupole('CHV', corr_len, 0.0, sext_pass_method);
qs = quadrupole('QS', corr_len, 0.0, quad_pass_method);
...
injsept_strength.kxl = injsept_kxl_strength;
injsept_strength.kyl = injsept_kyl_strength;
injsept_strength.ksxl = injsept_ksxl_strength;
injsept_strength.ksyl = injsept_ksyl_strength;
septin = segmented_septum(injsept_strength, 6);
...
qs = findcells(the_ring, 'FamName', 'QS');
...
the_ring = setcellstruct(the_ring, 'NumIntSteps', qs, 5);
the_ring = setcellstruct(the_ring, 'NumIntSteps', kicks, 1);

function sept_model = segmented_septum(strengths, nseg)
    if nseg < 2
