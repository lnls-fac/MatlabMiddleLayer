function [r, lattice_title, IniCond] = sirius_tb_lattice_sept(varargin)
...
% -- bo injection septum with gradient --
dip_nam =  'InjSept';
dip_len =  0.50;
dip_ang =  21.75 * deg_2_rad;
dip_K   =  0.0;
...
[~, ~, brho] = lnls_beta_gamma(0.15);
septine = rbend_sirius(dip_nam, dip_len/2, dip_ang/2, 1*dip_ang/2, 0*dip_ang, 0,0,0, [0,0,0], [0,dip_K,dip_S], bend_pass_method);
septins = rbend_sirius(dip_nam, dip_len/2, dip_ang/2, 0*dip_ang, 1*dip_ang/2, 0,0,0, [0,0,0], [0,dip_K,dip_S], bend_pass_method);
M = eye(6);
% The gradient is distributed in 3 parts: beginning, middle and end of
% septum
M(2, 1) = Gx * dip_len/ 3 / brho;
M(2, 3) = GSx * dip_len/ 3 / brho;
M(2, 3) = GSy * dip_len/ 3 / brho;
M(4, 3) = Gy * dip_len/ 3 / brho;
sept_grad = matrix66('SeptGrad', 0, M, 'Matrix66Pass');
bseptin = marker('bInjS','IdentityPass');
eseptin = marker('eInjS','IdentityPass');
septin  = [bseptin, sept_grad, septine, sept_grad, septins, sept_grad, eseptin]; % excluded ch to make it consistent with other codes. the corrector can be implemented in the polynomB.
