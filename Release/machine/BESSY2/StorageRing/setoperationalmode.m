...
%
%  See also aoinit, updateatindex, bessy2init
...
if isempty(ModeNumber)
    %ModeNumber = 1;
    ModeCell = {'BESSY2 Operations', 'BESSY2 Model'};
    [ModeNumber, OKFlag] = listdlg('Name','BESSY2','PromptString','Select the Operational Mode:', 'SelectionMode','single', 'ListString', ModeCell, 'ListSize', [450 200]);
    if OKFlag ~= 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
AD = getad;
AD.Machine = 'BESSY2';            % Will already be defined if setpathmml was used
...
AD.Energy = 1.7;                  % make sure this is the same as bend2gev at the production lattice!
AD.InjectionEnergy = 1.7;
AD.HarmonicNumber = 400;
...
if ModeNumber == 1
    % User mode
    AD.OperationalMode = 'BESSY 2';
    ModeName = 'User';
    OpsFileExtension = '_BESSY2';

    % AT lattice
    AD.ATModel = 'bessy2atdeck';
    bessy2atdeck;
    ...
    % This is a bit of a cluge to know if the user is on the BESSY2 filer
    % If so, then the MML user probably wants to be online
    MMLROOT = getmmlroot;
    if isempty(findstr(lower(MMLROOT),'SomeDirectoryAtBESSY2'))
        switch2sim;
    else
        switch2online;
        ...
        try
            if exist('lcaSetSeverityWarnLevel','file')
                % Retry count
                RetryCountNew = 200;  % was 1000  Default: 599-old labca, 149-labca_2_1_beta
                ...
                % Timeout
                %TimeoutNew = .005;  % was .005  Default: .05-old labca, .1-labca_2_1_beta
...
elseif ModeNumber == 101
    % Model mode
    AD.OperationalMode = 'BESSY 2 - Model';
    ModeName = 'Model';
