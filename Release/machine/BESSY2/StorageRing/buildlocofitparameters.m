...
try
    if isfield(LocoMeasData, 'MachineConfig')
        ...
        if all(LocoMeasData.MachineConfig.S4.Setpoint.Data == 0)
            fprintf('   Turning S4 family off in the LOCO model.\n');
            ATIndex = family2atindex('S4');
            ATIndex = ATIndex(:);
            for i = 1:length(ATIndex)
                RINGData.Lattice{ATIndex(i)}.PolynomB(3) = 0;
            end
...
switch SkewFit
    ...
    case 2  
        ...
        AT_Index = [family2atindex('S1'); family2atindex('S2'); family2atindex('S3'); family2atindex('S4');]';
        for loop = 1:size(AT_Index,1)
            N = N + 1;
            FitParameters.Params{N} = mkparamgroup(RINGData.Lattice, AT_Index(loop,:),'s');
            FitParameters.Values = [FitParameters.Values; 0];
