...
try
    ...
    AO.SkewQuad.AT.ATIndex = buildatindex(AO.SkewQuad.FamilyName, Indices.SkewQuad);
    AO.SkewQuad.Position = findspos(THERING, AO.SkewQuad.AT.ATIndex(:,1))';
catch
    warning('QF, QD, or SkewQuad family not found in the model.');
end
...
try
    ...
    AO.BEND.AT.ATIndex = [Indices.BEND(:) Indices.SplitBEND(:)];
    %AO.BEND.AT.ATIndex = [Indices.BEND(:) [Indices.SplitBEND(:); NaN*ones(length(Indices.BEND)-length(Indices.SplitBEND),1)]];
...
try   
    ...
    TwissData.alpha = [0.0926 0.0144]';
    TwissData.beta  = [9.6294 10.9093]';
    TwissData.mu    = [0 0]';
    TwissData.ClosedOrbit = [0 0 0 0]';
    TwissData.dP = 0;
    TwissData.dL = 0;
    TwissData.Dispersion  = [-0.1397  0.0132 0.0823 -0.0004]';
