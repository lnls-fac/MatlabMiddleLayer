...
if size(Sector,1) ~= 1
    for iSector = 1:size(Sector,1)
        ffgettbl(Sector(iSector,:), BPMFlag);
    end
    return;
end
...
for i = 2:length(Gaps)+1
   ...
   %setsp('HCM', HCM0+tableXold(i,2:3)', HCMList);  % this does not seem to be a good idea
   %setsp('VCM', VCM0+tableYold(i,2:3)', VCMList);  % need to linear fit the data if you do use this???
...
FileName = sprintf('id%02de%2.0f', 7.0, 10*getenergy);
...
% Save in text format
FileNameText = sprintf('id%02de%2.0f.txt', 7.0, 10*getenergy);
