...
Navg = 1;
IDVel = 3.33;
IDVelEPU = 2; %just used for setting horizontal to zero at start
FFDate = date;
FFClock = clock;
FFGeV = GeV;
...
load([GLOBAL_ALSDATA_DIRECTORY GLOBAL_SR_PRODUCTION_FILE]);

% load sector 11 EPU skew quadrupole table
load([GLOBAL_ALSDATA_DIRECTORY 'skew_table_epu11'])
...
for i = 2:length(Gaps)+1
   ...
   if strcmp(GLOBAL_SR_MODE_TITLE, '1.9 GeV, nuy = 9.20')
      if i==2
         disp('  ');
         disp('  Generating feedforward table for nuy = 9.20 lattice');
      end
      ...
      elseif strcmp(FFTypeFlag,'Global')
         ...
         DeltaAmps = inv(GLOBAL_SR_TUNE_MATRIX) * [(fraccorr*6.23e-4)-DeltaNuX;fraccorr*(-0.05301)];    %  DelAmps =  [QF; QD];
      ...
      if (Sector==7) | (Sector==10) | (Sector==11)
         DeltaAmps = inv(GLOBAL_SR_TUNE_MATRIX/12) * [0; -DeltaNuY];    %  DelAmps =  [QF; QD];
