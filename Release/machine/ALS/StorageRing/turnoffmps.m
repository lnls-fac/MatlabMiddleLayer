...
if EnergyBEND < .1 && max(EnergySBEND)>.25
    ...
    if strcmp(StartFlag,'No')
        fprintf('   Turning magnet power supplies off has been caneled.\n');
        return
    end
end
...
if EnergyBEND > .1
    ...
    if FailFlag
        error('Ramping to zero failed, hence power supply turn off caneled.');
    end
end
...
% Remove families
RemoveFamilyNames = {'SQEPU','HCMCHICANEM'};  %'VCMCHICANE',
