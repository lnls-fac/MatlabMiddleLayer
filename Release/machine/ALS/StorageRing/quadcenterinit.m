...
QMS.CorrectOrbit = 'No';   % 'yes' or 'no';  % Only select 'yes' if the orbit is reasonably close to the offset orbit 
QMS.ExtraDelay = 0;        % Eddy current has been put in setquad so ExtraDelay is not needed
...
if QMS.QuadPlane==1        
    ...
    switch CorrMethod
        ...
        case 'MEC'
            ...
            % Base MEC on the response matrix
            ...
            if 1
                ...
                if any(R==0)
                    ...
                    QMS.CorrDelta = MaxOrbit / R;  % +/- mm
                end
            end
            
        case 'LocalBump'
            % Local bump corrector method
            [OCS, RF, OCS0] = setorbitbump('BPMx',QMS.BPMDev,.1,'HCM',[-2 -1 1 2], 'NoSetSP');
...
elseif QMS.QuadPlane == 2
    ...
    switch CorrMethod
        ...
        case 'LocalBump'
            % Local bump corrector method
            [OCS, RF, OCS0] = setorbitbump('BPMy', QMS.BPMDev, .25, 'VCM', [-2 -1 1 2], 'NoSetSP');
            QMS.CorrDelta = OCS.CM.Data - OCS0.CM.Data;
