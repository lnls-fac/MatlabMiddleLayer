...
% Last Modified by GUIDE v2.5 19-Jan-2012 17:33:49
...
set(handles.radiobutton_Bump,'Value',0);
...
commonnames = family2common('BPMx', BPM.BPMx{1}.DeviceList); % XRR
BPMDeviceListString = '';
for i = 1:size(BPM.BPMx{1,1}.DeviceList,1)
    %BPMDeviceListString = strvcat(BPMDeviceListString, sprintf('%s[%d, %d]', 'BPM', BPMDeviceList(i,:)));
    BPMDeviceListString = strvcat(BPMDeviceListString, commonnames(i,:));
end
set(handles.PopUpInicioBump, 'String', BPMDeviceListString);
set(handles.PopUpFimBump, 'String', BPMDeviceListString);

commonnames = family2common('BPMy', BPM.BPMy{1}.DeviceList); % XRR
BPMDeviceListString = '';
for i = 1:size(BPM.BPMy{1}.DeviceList,1)
    %BPMDeviceListString = strvcat(BPMDeviceListString, sprintf('%s[%d, %d]', 'BPM', BPMDeviceList(i,:)));
    BPMDeviceListString = strvcat(BPMDeviceListString, commonnames(i,:));
end
set(handles.VPopUpInicioBump, 'String', BPMDeviceListString);
set(handles.VPopUpFimBump, 'String', BPMDeviceListString);
...
OCS.CorrectorGain = str2num(get(handles.CorrectorGain,'String'))/100;
...
if get(hObject,'Value')
    ...
    set(handles.SelectHCM,  'Enable', 'On')
    set(handles.GetOrbit,   'Enable', 'On')
    set(handles.SizeBump,   'Enable', 'On')
    set(handles.PopUpInicioBump,   'Enable', 'On')
    set(handles.PopUpFimBump,   'Enable', 'On')
else
    set(handles.SelectBPMx, 'Enable', 'Off')
    set(handles.SelectHCM,  'Enable', 'Off')
    set(handles.SizeBump,   'Enable', 'Off')
    set(handles.PopUpInicioBump,   'Enable', 'Off')
    set(handles.PopUpFimBump,   'Enable', 'Off')
...
if get(handles.VerticalPlane,'Value')
    ...
    set(handles.SelectVCM,  'Enable', 'On')
    set(handles.GetOrbit,   'Enable', 'On')
    set(handles.VSizeBump,   'Enable', 'On')
    set(handles.VPopUpInicioBump,   'Enable', 'On')
    set(handles.VPopUpFimBump,   'Enable', 'On')
else
    set(handles.SelectBPMy, 'Enable', 'Off')
    set(handles.SelectVCM,  'Enable', 'Off')
    set(handles.VSizeBump,   'Enable', 'Off')
    set(handles.VPopUpInicioBump,   'Enable', 'Off')
    set(handles.VPopUpFimBump,   'Enable', 'Off')
end
...
if iscell(DataOriginal)
    ...
    CheckList(iDev) = 1;
    % Data.DeviceList = editlist(DeviceListTotal, Data.FamilyName, CheckList); % XRR
    commonnames = dev2common(Data.FamilyName, DeviceListTotal); % XRR
    DataDeviceList = editlist(commonnames, '', CheckList);
    Data.DeviceList = common2dev(DataDeviceList, Family); % XRR
...
BPMDeviceListString = '';
commonnames = dev2common(Family, Data.DeviceList);
for i = 1:size(Data.DeviceList,1)
    %BPMDeviceListString = strvcat(BPMDeviceListString, sprintf('%s[%d, %d]', 'BPM', BPMDeviceList(i,:)));
    BPMDeviceListString = strvcat(BPMDeviceListString, commonnames(i,:));
end
...
if ~get(handles.radiobutton_Bump,'Value');
...
if ~isempty(handles)
    ...
    elseif strcmpi(get(handles.Plots6,'Checked'),'On')
        ...
        set(handles.axes4, 'Position', a + [0 .6 0 0]);
...
% --- Executes on button press in radiobutton_Bump.
function radiobutton_Bump_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_Bump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_Bump
interruptor = get(handles.radiobutton_Bump,'Value');
OCS = getappdata(handles.figure1, 'OCS');
inicio = get(handles.PopUpInicioBump,'Value');
final = get(handles.PopUpFimBump,'Value');
bump = str2num(get(handles.SizeBump,'string'));
vinicio = get(handles.VPopUpInicioBump,'Value');
vfinal = get(handles.VPopUpFimBump,'Value');
vbump = str2num(get(handles.VSizeBump,'string'));

if interruptor
    set(handles.SizeBump,'Enable','Off');
    if inicio>final
        OCS.GoalOrbit{1}(1:inicio) = OCS.GoalOrbit{1}(1:inicio)+bump;
        OCS.GoalOrbit{1}(final:end) = OCS.GoalOrbit{1}(final:end)+bump;
    else
        OCS.GoalOrbit{1}(inicio:final) = OCS.GoalOrbit{1}(inicio:final)+bump;
    end
        set(handles.VSizeBump,'Enable','Off');
    if vinicio>vfinal
        OCS.GoalOrbit{2}(1:vinicio) = OCS.GoalOrbit{2}(1:vinicio)+vbump;
        OCS.GoalOrbit{2}(vfinal:end) = OCS.GoalOrbit{2}(vfinal:end)+vbump;
    else
        OCS.GoalOrbit{2}(vinicio:vfinal) = OCS.GoalOrbit{2}(vinicio:vfinal)+vbump;
    end

else
    set(handles.SizeBump,'Enable','On');
    if inicio>final
        OCS.GoalOrbit{1,1}(1:inicio) = OCS.GoalOrbit{1,1}(1:inicio)-bump;
        OCS.GoalOrbit{1,1}(final:end) = OCS.GoalOrbit{1,1}(final:end)-bump;
    else
        OCS.GoalOrbit{1,1}(inicio:final) = OCS.GoalOrbit{1,1}(inicio:final)-bump;
    end
    set(handles.VSizeBump,'Enable','On');
    if vinicio>vfinal
        OCS.GoalOrbit{2}(1:vinicio) = OCS.GoalOrbit{2}(1:vinicio)-vbump;
...
% --- Executes on selection change in PopUpMenu_Iterations.
function PopUpMenu_Iterations_Callback(hObject, eventdata, handles)
% hObject    handle to PopUpMenu_Iterations (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns PopUpMenu_Iterations contents as cell array
%        contents{get(hObject,'Value')} returns selected item from PopUpMenu_Iterations



function SizeBump_Callback(hObject, eventdata, handles)
% hObject    handle to SizeBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of SizeBump as text
%        str2double(get(hObject,'String')) returns contents of SizeBump as a double


% --- Executes during object creation, after setting all properties.
function SizeBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SizeBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% --- Executes during object creation, after setting all properties.
function radiobutton_Bump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to radiobutton_Bump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function PopUpInicioBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PopUpInicioBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function PopUpFimBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PopUpFimBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function SelectBPMx_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SelectBPMx (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in PopUpInicioBump.
function PopUpInicioBump_Callback(hObject, eventdata, handles)
% hObject    handle to PopUpInicioBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns PopUpInicioBump contents as cell array
%        contents{get(hObject,'Value')} returns selected item from PopUpInicioBump


% --- Executes on selection change in PopUpFimBump.
function PopUpFimBump_Callback(hObject, eventdata, handles)
% hObject    handle to PopUpFimBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns PopUpFimBump contents as cell array
%        contents{get(hObject,'Value')} returns selected item from PopUpFimBump



function VSizeBump_Callback(hObject, eventdata, handles)
% hObject    handle to VSizeBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of VSizeBump as text
%        str2double(get(hObject,'String')) returns contents of VSizeBump as a double


% --- Executes during object creation, after setting all properties.
function VSizeBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to VSizeBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in VPopUpInicioBump.
function VPopUpInicioBump_Callback(hObject, eventdata, handles)
% hObject    handle to VPopUpInicioBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns VPopUpInicioBump contents as cell array
%        contents{get(hObject,'Value')} returns selected item from VPopUpInicioBump


% --- Executes during object creation, after setting all properties.
function VPopUpInicioBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to VPopUpInicioBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in VPopUpFimBump.
function VPopUpFimBump_Callback(hObject, eventdata, handles)
% hObject    handle to VPopUpFimBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns VPopUpFimBump contents as cell array
%        contents{get(hObject,'Value')} returns selected item from VPopUpFimBump


% --- Executes during object creation, after setting all properties.
function VPopUpFimBump_CreateFcn(hObject, eventdata, handles)
% hObject    handle to VPopUpFimBump (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
