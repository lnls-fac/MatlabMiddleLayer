...
%
%  2009-02-03 Ximenes R Resende & Lucas Fantini de Lima
%     'Length' added as a possible parameter group
%
%  2009-02-26 Ximenes R Resende
%     'Bmax' added as a posible parameter group
...
switch lower(PARAMSTR)
    ...
    case 'pb1'
     ...
     case {'length'}
        if ~isfield(LATTICE{INDEX(KIDNUM(1))},'Length')
            error(['Element ',int2str(KIDNUM(1)),' does not have field ''Length''']);
        end
        P = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','Length','Function',inline('x'));
        [P.FieldIndex]=deal({1,1});
        [P.Args]=deal({});
        for k = 1:length(KIDNUM)
            P(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.Length;
        end
        
     case {'bmax'}
        if ~isfield(LATTICE{INDEX(KIDNUM(1))},'Bmax')
            error(['Element ',int2str(KIDNUM(1)),' does not have field ''Bmax''']);
        end
        P = struct('ElemIndex',num2cell(INDEX(KIDNUM)),'FieldName','Bmax','Function',inline('x'));
        [P.FieldIndex]=deal({1,1});
        [P.Args]=deal({});
        for k = 1:length(KIDNUM)
            P(k).SavedValue = LATTICE{INDEX(KIDNUM(k))}.Bmax;
        end
end
