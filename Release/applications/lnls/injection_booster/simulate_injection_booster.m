function simulate_injection_booster(ring, n_part, name_bpm, name_scrn, n_scrn, kckr)
...
if exist('name_scrn','var') && ~exist('n_scrn','var'), error('Escolha uma screen entre 1, 2 e 3'); end

if name_scrn
    
    if n_scrn == 0 || n_scrn > 3
        error('Escolha uma screen entre 1, 2 e 3')
    end
    ...
    r_final = sirius_booster_injection(machine, param, n_part, kckr);
   
    sigma_scrn = param.sigma_scrn;    
    r_scrn = r_final([1,3], :, scrn);
    sigma_scrn_x = lnls_generate_random_numbers(1, length(sigma_scrn), 'norm') .* sigma_scrn;
    sigma_scrn_y = lnls_generate_random_numbers(1, length(sigma_scrn), 'norm') .* sigma_scrn;
    ...
    dtheta0 = ajustes_injecao(machine, r_scrn(1));
    
    param.offset_xl = param.offset_xl + dtheta0;
    
    r_final = graficos(machine, r_final);
    % return
end

if name_bpm
    r_final = sirius_booster_injection(machine, param, n_part, kckr);
    [~, bpm] = bpms(machine, r_final([1,3], :, :), param.sigma_bpm);
end

    function r_final = graficos(machine, r_final)
