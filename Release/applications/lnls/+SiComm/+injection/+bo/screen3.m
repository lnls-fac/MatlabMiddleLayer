function [param, r_scrn3] = screen3(machine, param, param_error, n_part, n_pulse, scrn3, kckr)
    ...
    y_scrn3 = 1; res_scrn = param_error.sigma_scrn_pulse;
    res_x = sqrt(res_scrn^2 + (param.etax_scrn3 * param_error.delta_error_pulse)^2);

    r_particles = sirius_commis.injection.bo.multiple_pulse(machine3, param, param_error, n_part, n_pulse, scrn3, kckr);
    eff_lim = 0.50;
    ...
    if mean(eff3) < eff_lim
        param = sirius_commis.injection.bo.screen_low_intensity(machine3, param, param_error, n_part, n_pulse, scrn3, kckr, mean(eff3), 3, eff_lim);
        r_particles = sirius_commis.injection.bo.multiple_pulse(machine3, param, param_error, n_part, n_pulse, scrn3, kckr);
        r_scrn3 = r_particles.r_screen;
    end
    ...
    if abs(x_scrn3) < res_x % && abs(y_scrn3) < res_scrn
        ...
        agr = sirius_commis.common.prox_percent(param.delta_ave, param_error.delta_error_syst);
        fprintf('THE GENERATED ERROR WAS %f %%, Conf. %f %% \n', param_error.delta_error_syst*1e2, agr);
        fprintf('=================================================\n');
        return
    end
