%lnls_touschek_lifetime(data_at,phase,I,Nb,K): Calculates the Touschek
...
%    Nb is different from 1, input must be total beam current and uniform 
...
function Result = lnls_touschek_lifetime(data_at,phase,I,Nb,K)
...
data_at_new = lnls_effect_emittance(data_at,phase);
...
[emit_ibs,~,~,~] = lnls_calc_ibs_bane_cimp(data_at_new,Ib,K);
...
%Calculates the Touschek lifetime to each 1 cm of ring:
npoints = ceil((Accep.s(end) - Accep.s(1))/0.01);
...
Dp = interp1(range(:),dtouschek(:),ksip,'linear',0.0);
Dn = interp1(range(:),dtouschek(:),ksin,'linear',0.0);
