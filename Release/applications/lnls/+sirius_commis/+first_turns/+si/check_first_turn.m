function [machine_correct, first_turn_mach, first_turn_mach_refined, count_turns, gr_mach] = check_first_turn(machine, n_mach, param, param_errors, n_part, n_pulse, n_turns, n0, n_sv, M_acc)
...
count_turns = ones(3, n_mach) * n_turns;
...
for j = 1:n_mach    
    ...
    if n_min < n_turns
        ...
        machine_correct{j} = sirius_commis.first_turns.si.first_turn_corrector(machine, 1, param, param_errors, M_acc, n_part, 10, n_sv);
        machine_correct{j} = machine_correct{j}{1};
        first_turn_mach(j) = 1;
        [count_turns(:, j), r_bpm, int_bpm] = sirius_commis.first_turns.si.multiple_pulse_turn(machine_correct{j}, 1, param, param_errors, n_part, 3, n_turns);
        ...
        %    count_turns(:, j) = sirius_commis.first_turns.si.multiple_pulse_turn(machine_correct{j}, 1, param, param_errors, n_part, n_pulse, n_turns);
        %end
    end
end
end
