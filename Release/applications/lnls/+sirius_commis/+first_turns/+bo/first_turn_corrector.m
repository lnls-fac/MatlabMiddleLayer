...
for j = 1:n_mach
    ...
    if flag_tl
        [machine_cell{j}, theta_x{j}, theta_y{j}, rms_orbit_bpm(j, :), max_orbit_bpm(j, :)] = sirius_commis.first_turns.bo.correct_orbit_bpm_tl(machine, param, M_acc, n_part, n_pulse);
    else
        [machine_cell{j}, theta_x{j}, theta_y{j}, rms_orbit_bpm(j, :), max_orbit_bpm(j, :)] = sirius_commis.first_turns.bo.correct_orbit_bpm_matrix(machine, param, M_acc, n_part, n_pulse);
