function M = calc_respm_tracking(bo_ring)
...
for i = 1:n_ch
    bo_ring_plus = lnls_set_kickangle(bo_ring, delta_kick/2, fam.CH.ATIndex(i), 'x');
    bo_ring_minus = lnls_set_kickangle(bo_ring, - delta_kick/2, fam.CH.ATIndex(i), 'x');
    ...
    M(1:n_bpm, i) = Delta_R(1, fam.BPM.ATIndex) ./ delta_kick;
    M(n_bpm +1:2*n_bpm, i) = Delta_R(3, fam.BPM.ATIndex) ./ delta_kick;
end

for j = 1:n_cv
    bo_ring_plus = lnls_set_kickangle(bo_ring, delta_kick/2, fam.CV.ATIndex(j), 'y');
    bo_ring_minus = lnls_set_kickangle(bo_ring, - delta_kick/2, fam.CV.ATIndex(j), 'y');
    R_out_plus = linepass(bo_ring_plus, R_in, 1:len_ring + 1);
    R_out_minus = linepass(bo_ring_minus, R_in, 1:len_ring + 1);
    Delta_R = (R_out_plus - R_out_minus);
    M(1:n_bpm, n_ch + j) = Delta_R(1, fam.BPM.ATIndex) ./ delta_kick;
    M(n_bpm+1:2*n_bpm, n_ch + j) = Delta_R(3, fam.BPM.ATIndex) ./ delta_kick;
end
