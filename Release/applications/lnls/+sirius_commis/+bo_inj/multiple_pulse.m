...
% sirius_commis.common.initializations();
    ...
    if ismember(point, scrn)
        off_scrn = 0; % param_errors.offset_scrn(:, point == scrn);
    else
        off_scrn = 0;
    end
    ...
    if(exist('plt','var'))
        ...
        elseif(strcmp(plt,'diag'))
            flag_diag = true;
            flag_plot = false;
            if ~isfield(param, 'orbit')
                orbit = findorbit(machine, 0, 1:length(machine));
            else
                orbit = param.orbit;
            end
        else
    ...
    for j=1:n_pulse
        ...
        r_particles = sirius_commis.bo_inj.single_pulse(machine, param, n_part, point);
    ...
    % Beam shape plot with colormap (not very trustable)
    % sirius_commis.scatplot(1e3 * squeeze(r_xy(1, :, end)), 1e3 * squeeze(r_xy(2, :, end)), 'squares', 0.1, 500, 5, 1, 4); % , 1, 1e3, 5, 1, 4);
    
    % Beam shape plot without colormap and sizes based on gaussian fitting
    figure;
    plot(1e3 * squeeze(r_xy(1, :, end)), 1e3 * squeeze(r_xy(2, :, end)), '.');
    % Vaccum chamber aperture
    xlim([-18, 18]);
    ylim([-18, 18]);
    grid on
    
    x = squeeze(r_xy(1, :, end));
    x = x(~isnan(x));
    fx = fitdist(x', 'Normal');
    xsize = fx.sigma;
    fprintf('Horizontal Size %f mm \n', xsize*1e3);
    
    y = squeeze(r_xy(2, :, end));
    y = y(~isnan(y));
    fy = fitdist(y', 'Normal');
    ysize = fy.sigma;
    fprintf('Vertical Size %f mm \n', ysize*1e3);
    ...
    if flag_diag
        ...
        r_bpm = sirius_commis.common.compares_vchamb(machine, r_bpm_mean, bpm, 'bpm'); % Vacuum chamber comparison
