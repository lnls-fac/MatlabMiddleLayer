function sirius_cod_stat(N, arq_machines, scale)
...
%                  de testes apenar com erros dinamicos 1e9. 
    ...
    % end
    thering0 = sirius_si_lattice();
    twiss0 = calctwiss(thering0);
    sext_idx = findcells(machine_corr{1},'PolynomB');
    % Armazena todos os valoes de twiss das N maquina num vetor de cells
    for i=1:N
        if scale==1e3
            % sext_idx = getappdata(0, 'Sextupole_Idx');
            % sext_str = getcellstruct(machine_corr{i}, 'PolynomB', sext_idx, 1, 3);
            % machine_corr{i} = set_sextupoles(machine_corr{i}, 0, sext_str);
            polynomb_orig = getcellstruct(machine_corr{i}, 'PolynomB', sext_idx);
            for k=1:length(polynomb_orig)
                PolyB = machine_corr{i}{sext_idx(k)}.PolynomB; % necessary so that correctors kicks are not lost
                PolyB(3:end) = polynomb_orig{k}(3:end) * 0;
                machine_corr{i} = setcellstruct(machine_corr{i},'PolynomB',sext_idx(k),{PolyB});
            end
            % machine_corr{i} = sirius_commis.first_turns.si.turn_off_sext(machine_corr{i});
            twiss(i)=calctwiss(machine_corr{i});
        else
            twiss(i)=calctwiss(machine_corr{i});
        end
    end
    ...
    fam_data = sirius_si_family_data(machine_corr{1});
    bpm = fam_data.BPM.ATIndex;
    ch = fam_data.CH.ATIndex;
    cv = fam_data.CV.ATIndex;
    ...
    for i=1:length(twiss0.cox)
        ...
        codx_max(i)=nanmax(abs(posx))*scale;
        codx_std(i)=nanstd(posx)*scale;
        cody_max(i)=nanmax(abs(posy))*scale;
        cody_std(i)=nanstd(posy)*scale;      
    end
    ...
    for i=1:N
        kick_x(i)= max(lnls_get_kickangle(machine_corr{i}, ch, 'x')) * scale; %max(abs(getcellstruct(twiss(i).THERING,'KickAngle',ch,1)))*scale;
        kick_y(i)= max(lnls_get_kickangle(machine_corr{i}, cv, 'y')) * scale; % max(abs(getcellstruct(twiss(i).THERING,'KickAngle',cv,2)))*scale;
        std_kickx(i)= std(lnls_get_kickangle(machine_corr{i}, ch, 'x')) * scale; % std(getcellstruct(twiss(i).THERING,'KickAngle',ch,1))*scale;
        std_kicky(i)= std(lnls_get_kickangle(machine_corr{i}, cv, 'y')) * scale;   % std(getcellstruct(twiss(i).THERING,'KickAngle',cv,2))*scale;
    ...
    for i=1:N
        beatx(i) = nanmean(abs(twiss0.betax-twiss(i).betax)./twiss0.betax)*100;
        beaty(i) = nanmean(abs(twiss0.betay-twiss(i).betay)./twiss0.betay)*100;
    ...
    for i=1:N
        ...
        maxBCx(i)  = nanmax(abs(twiss(i).cox))*scale;
        stdBCx(i)  = nanstd(twiss(i).cox)*scale;
        codx_sigmax(i)  = nanmax(abs(twiss(i).cox)./sqrt(0.2772*1e-9*twiss(i).betax+0.00083^2*twiss(i).etax.^2));
        codxp_sigmaxp(i)  = nanmax(abs(twiss(i).coxp)./sqrt(0.2772*1e-9*(1+twiss(i).alphax.^2)./twiss(i).betax+0.00083^2*twiss(i).etaxl.^2));
        maxBCy(i)  = nanmax(abs(twiss(i).coy))*scale;
        stdBCy(i)  = nanstd(twiss(i).coy)*scale;
        cody_sigmay(i)  = nanmax(abs(twiss(i).coy)./sqrt(0.0028*1e-9*twiss(i).betay));
        codyp_sigmayp(i)  = nanmax(abs(twiss(i).coyp)./sqrt(0.0028*1e-9*(1+twiss(i).alphay.^2)./twiss(i).betay));
