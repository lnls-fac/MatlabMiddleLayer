function [param, x_bpm, delta_bpm] = bpm2_energy(machine, param, param_errors, n_part, n_pulse, bpm, kckr)
    ...
    [eff, r_bpms] = sirius_commis.injection.si.multiple_pulse(machine, param, param_errors, n_part, n_pulse, bpm, kckr, 'plot');
    eff_lim = 0.75;

    if mean(eff) < eff_lim
        param = sirius_commis.injection.si.bpm_low_intensity(machine, param, param_errors, n_part, n_pulse, bpm, kckr, mean(eff), 3, eff_lim);
        [~, r_bpms] = sirius_commis.injection.si.multiple_pulse(machine, param, param_errors, n_part, n_pulse, bpm, kckr, 'plot');
    end
    ...
    if abs(x_bpm3) < res || difs < res % abs(x_bpm2) < res2 && abs(x_bpm3) < res3
        fprintf('BPM 2 - x position %f mm \n', x_bpm2*1e3);
        fprintf('BPM 3 - x position %f mm \n', x_bpm3*1e3);
        fprintf('=================================================\n');
        fprintf('DELTA ENERGY ADJUSTED TO %f %% \n', param.delta_ave_f*1e2);
        agr = sirius_commis.common.prox_percent(param.delta_ave_f, param_errors.delta_error_sist);
    ...
    dtheta_kckr = param.kckr_sist * ((param.delta_ave_i + 1) / (param.delta_ave_f + 1) - 1);
    % dtheta_sept = param.offset_xl_sist * (param.delta_ave_f - param.delta_ave_i) / (1 + param.delta_ave_f);
    % fprintf('DELTA THETA KICKER %f urad \n', dtheta_kckr * 1e6)
    param.kckr_sist = param.kckr_sist + dtheta_kckr;
    % param.offset_xl_sist = param.offset_xl_sist - dtheta_sept;
    fprintf('BPM 2 - x position %f mm \n', x_bpm2*1e3);
    fprintf('BPM 3 - x position %f mm \n', x_bpm3*1e3);
    fprintf('=================================================\n');
    fprintf('DELTA ENERGY ADJUSTED TO %f %% \n', param.delta_ave_f*1e2);
