function [r_scrn2, param] = screen2(machine, param, param_errors, n_part, n_pulse, scrn2, kckr)
    ...
    while abs(dx12) > res_scrn % || abs(dyf_new) > 2 * res_scrn % / sqrt(n_pulse)
        param.kckr_sist = param.kckr_sist - dtheta_kckr;
        % param.offset_yl_sist = param.offset_yl_sist - dthetay;
        param.offset_y_sist = param.offset_y_sist - p * dyf_new;
        ...
        if abs(dyf_new) > abs(dyf_old)
           p = 0;
           param.offset_y_sist = param.offset_y_sist + dyf_old;
        end
