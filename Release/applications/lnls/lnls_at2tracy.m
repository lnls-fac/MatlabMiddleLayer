function lnls_at2tracy(the_ring, file_name)
...
%the_ring = change_fnames_different_elements(the_ring);
...
for j=1:length(uident)
    ...
    if isfield(the_ring{i}, 'K') && ~isfield(the_ring{i}, 'BendingAngle')
        fprintf(fp, ['%-' int2str(max_length) 's : quadrupole, l = %9.6f, k = %+9.6f, n = %i, method = meth;' endofline], FamName, the_ring{i}.Length, the_ring{i}.K, the_ring{i}.NumIntSteps);
        nr_elems = nr_elems + 1;
        continue;
    end
    if isfield(the_ring{i}, 'PolynomB') && ~isfield(the_ring{i}, 'K') && ~isfield(the_ring{i}, 'BendingAngle')
        fprintf(fp, ['%-' int2str(max_length) 's : sextupole, l = %9.6f, k = %+9.6f, n = %i, method = meth;' endofline], FamName, the_ring{i}.Length, the_ring{i}.PolynomB(3), the_ring{i}.NumIntSteps);
        nr_elems = nr_elems + 1;
        continue;
    end
    ...
    if isfield(the_ring{i}, 'BendingAngle')
        fprintf(fp, ['%-' int2str(max_length) 's : multipole, l = %9.6f, t = %11.6f, n = %i, method = meth, ' endofline],  FamName, the_ring{i}.Length, (180/pi)* the_ring{i}.BendingAngle, the_ring{i}.NumIntSteps);
...
function the_ring = change_fnames_different_elements(the_ring0)

% a ser completado...

the_ring = the_ring0;

fnames = unique(getcellstruct(the_ring, 'FamName', 1:length(the_ring)));
   ...
   idx_ref = idx;
    ...
    for j=1:length(idx)
        ...
        for k=1:length(unique_idx)
            if same_elements(the_ring{idx(j)}, the_ring{unique_idx(k)})
                clone_idx = unique_idx(k);
            end
        end
        if clone_idx == 0
            unique_idx(end+1) = idx(j);
            idx_ref(j) = idx(j);
            %the_ring{idx(j)}.FamName = [fnames{i} num2str(counter, '%02i')];
            counter = counter + 1;
        else
            idx_ref(j) = clone_idx;
            %the_ring{idx(j)}.FamName = the_ring{clone_idx}.FamName;
        end
    end
    
    if length(unique_idx) > 1
        for j=1:length(unique_idx)
            sel = (idx_ref == unique_idx(j));
...
function r = same_elements(e1, e2)
    ...
    for i=1:length(f)
        if ~isfield(e2, f{i})
            r = false;
            return;
        elseif strcmpi(f{i}, 'twiss')
            continue;
        elseif strcmpi(f{i}, 'FamName')
            continue;
        elseif isstruct(e1.(f{i}))
            r = same_elements(e1.(f{i}), e2.(f{i}));
            if r == false, return; end;
        elseif ischar(e1.(f{i}))
            r = strcmp(e1.(f{i}), e2.(f{i}));
            if r == false, return; end;
        else
            r = (e1.(f{i}) == e2.(f{i}));
            if r == false, return; end;
