    ...
    if isempty(ConfigSetpoint)
        ...
        for i = 1:size(RestoreFamilyTable,1)
            if ~RestoreFamilyTable{i,3}
                ...
                if isempty(fieldnames(ConfigSetpoint.(RestoreFamilyTable{i,1})))
    ...
    if isempty(ConfigMonitor)
        ...
        for i = 1:size(MonitorFamilyTable,1)
            if ~MonitorFamilyTable{i,3}
                ...
                if isempty(fieldnames(ConfigMonitor.(MonitorFamilyTable{i,1})))
        ...
        for i = 1:length(Families)
            ...
            for j = 1:length(Fields)
                ...
                for k = 1:size(ChannelName,1)
                    n = n + 1;
                    if length(Fields) > 1
                        RestoreDataTable{n,1} = [deblank(CommonName(k,:)) ' (', deblank(Fields{j}), ')'];
                    else
                        RestoreDataTable{n,1} = deblank(CommonName(k,:));
                    end
        ...
        for i = 1:length(Families)
            ...
            for j = 1:length(Fields)
                ...
                for k = 1:size(ChannelName,1)
                    n = n + 1;
                    if length(Fields) > 1
                        MonitorDataTable{n,1} = [deblank(CommonName(k,:)) ' (', deblank(Fields{j}), ')'];
                    else
                        MonitorDataTable{n,1} = deblank(CommonName(k,:));
                    end
