...
if size(ChannelNames,1) > 0    
    ...
    if strcmpi(DataType, 'String')
        error('LNLS1Link does not return strings at the moment.');
        
        
    elseif strcmpi(DataType, 'Vector')

        % Get data
        AM = getpvsirius(ChannelNames(1,:));
        AM = AM(:);
        if N > 0
            AM = AM(1:N);
        end
    ...
    elseif any(strcmpi(DataType, {'Matrix','Waveform'}))
        ...
        for k = 1:size(ChannelNames,1)
            tmp = getpvsirius(ChannelNames(k,:));
            AM(k,:) = tmp(:)';
        end
        tout = etime(clock, t0);
        ...
        for itime = 1:length(t)
            ...
            % LNLS1Link (double outputs)
            
            AM = [AM getpvsirius(ChannelNames)];
...
function Data =  getpvsirius(ChannelNames)
...
% sirius_comm_read(msg);
Data = zeros(length(modChannelNames),1);
% AD = getad;
ca_context = getappdata(0, 'ca_context');
for i=1:length(modChannelNames)
    Data(i,1) = Channels.create(ca_context, ChannelDescriptor('double', modChannelNames{i}, true));
%    Data(i,1) = AD.PVServer.state.(modChannelNames{i});
end
