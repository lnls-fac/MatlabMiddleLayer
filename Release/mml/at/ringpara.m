...
%changed on 2-Jul-2008 by Marc Munoz
...
dpindex = findcells(THERING,'ByError'); %Only bends have ByError
%[tw,tune,chrom] = twissring(THERING,0,dpindex,'chrom',0.00001);
[tw,tune,chrom] = twissring(THERING,0,1:length(THERING),'chrom',0.00001); %that gets the ring tunes
tw=tw(dpindex);
beta = cat(1, tw.beta);
...
if nargout == 0
    ...
    fprintf('   Momentum Compaction Factor: \t%8.8f\n', rp.alphac);
