    ...
    for i = 1:size(Family,1)
        for j = 1:size(Family,2)
            if iscell(StatusFlag) && iscell(PowerSupplyFlag)
                DeviceList{i,j} = family2dev(Family{i}, StatusFlag{i}, PowerSupplyFlag{i});
            elseif iscell(StatusFlag)
                DeviceList{i,j} = family2dev(Family{i}, StatusFlag{i}, PowerSupplyFlag);
            elseif iscell(PowerSupplyFlag)
                DeviceList{i,j} = family2dev(Family{i}, StatusFlag, PowerSupplyFlag{i});
            else
                DeviceList{i,j} = family2dev(Family{i}, StatusFlag, PowerSupplyFlag);
            end
        end
    end
    return
