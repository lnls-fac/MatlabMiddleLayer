function sirius_coupling_feedback(varargin)

close all; drawnow;

% --- PARAMS: nominal machine
config.lattice_version = 'SI.V20.01';
config.target_coupling = 0.03;
config.fname_nominal_machine = 'nominal_SI.V20.01_coup3p0.mat';

config.sim_anneal.qs_delta = 0.001/2;
config.sim_anneal.nr_iterations = 0*10000;
config.sim_anneal.scale_tilt = 0.5 * (pi / 180); 
config.sim_anneal.scale_sigmay = 0.5 * 1e-6;
config.target_id_coupling = 0.01;

%config.b2_selection = logical(repmat([1,0,1,0, 1,0,1,0],1,5)); % 20 B2
config.b2_selection = logical(repmat([1,0,0,0, 1,0,0,0],1,5)); % 10 B2
%config.b2_selection = logical(repmat([1,0,0,0, 0,0,0,0],1,5)); % 05 B2
%config.b2_selection = logical([[1,0,0,0, 0,0,0,0], repmat([0,0,0,0, 0,0,0,0],1,4)]); % 01 B2
...
r = load_model(config.lattice_version);

% --- generate nominal machine with controled coupling
[r.nominal, r.indices] = create_nominal_machine(r, config);

% --- sets polynom_a scales for IDs ---
if exist('ids.mat','file')
    data = load('ids.mat'); ids = data.ids;    
else
    ids = find_coupling_polynom_a_strength(r, config.target_id_coupling);
    save('ids.mat', 'ids'); 
end
...
function [nominal, indices] = create_nominal_machine(r, config)

if exist(config.fname_nominal_machine, 'file')
    data = load(config.fname_nominal_machine); nominal = data.nominal;
else
    nominal.the_ring = r.the_ring0;
    nominal.target_coupling = config.target_coupling;
    nominal.fname_nominal_machine = config.fname_nominal_machine;
    nominal.b2_selection = config.b2_selection;
    nominal.sim_anneal = config.sim_anneal;
    % calcs nominal sigmay
    tw = calctwiss(r.the_ring0);
    tw.gammax = (1 + tw.alphax.^2) ./ tw.betax;
    tw.gammay = (1 + tw.alphay.^2) ./ tw.betay;
    as = atsummary(r.the_ring0);
    ex = as.naturalEmittance / (1 + nominal.target_coupling);
    ey = nominal.target_coupling * as.naturalEmittance / (1 + nominal.target_coupling);
...
if exist('b2_selection','var')
    indices = find_indices(r.the_ring0, b2_selection);
else
    indices = find_indices(r.the_ring0);
end

% --- searches for nominal machine ---
[r1, c1] = calc_residue(nominal.the_ring, indices, config.sim_anneal, nominal);
fprintf('residue of nominal the_ring\n');
fprintf('%04i: %f\n', 0, r1);
[f1,p1,f2,p2] = update_sim_annel_plots(c1,indices, nominal);
for i=1:config.sim_anneal.nr_iterations
    drawnow;
    the_ring = vary_qs_in_families(nominal.the_ring, indices, config.sim_anneal);
    [r2, c2] = calc_residue(the_ring, indices, config.sim_anneal, nominal);
    if (r2 < r1)
       r1 = r2; c1 = c2; nominal.the_ring = the_ring;
       fprintf('%04i: %f\n', i, r1); 
       [f1,p1,f2,p2] = update_sim_annel_plots(c1,indices,nominal,f1,p1,f2,p2);
       save(nominal.fname_nominal_machine,'nominal');
    end
end

nominal.coupling = calc_coupling(nominal.the_ring, indices);

function r = load_model(lattice_version)
...
r.the_ring0 = setcavity('On', the_ring);
r.lattice_version = lattice_version;
...
for i=1:length(machines.machine)
    fprintf('loading random machine %03i\n', i);
...
function machines = add_dqs_to_random_machines(machines0, nominal, indices)
...
for i=1:length(machines.machine)
    figure(f1);
    plot(indices.pos, (180/pi)*machines.coupling{i}.tilt, 'Color', [1.0,0.6,0.6]); drawnow
end
for i=1:length(machines.machine)
    figure(f1);
    scatter(indices.pos(indices.all), (180/pi)*machines.coupling{i}.tilt(indices.all), 50, [1.0,0.4,0.4], 'filled'); drawnow;
end
figure(f1);
scatter(indices.pos(indices.all), (180/pi)*nominal.coupling.tilt(indices.all), 50, [1.0,0.0,0.0], 'filled'); drawnow;
...
for i=1:length(machines.machine)
    figure(f1);
    plot(indices.pos, 1e6*machines.coupling{i}.sigmas(2,:), 'Color', [0.6,0.6,1.0]); drawnow
end
for i=1:length(machines.machine)
    figure(f1);
    scatter(indices.pos(indices.all), 1e6*machines.coupling{i}.sigmas(2,indices.all), 50, [0.4,0.4,1], 'filled'); drawnow;
end
figure(f1);
scatter(indices.pos(indices.all), 1e6*nominal.coupling.sigmas(2,indices.all), 50, [0.0,0.0,1], 'filled'); drawnow;
