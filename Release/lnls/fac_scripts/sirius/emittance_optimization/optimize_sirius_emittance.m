...
opt.stepsize_len = 0.01 * 10;
opt.stepsize_fld = 0.01 * 10;
opt.stepsize_grd = 0.01 * 10;
opt.energy       = 3;
opt.nr_cells     = 20;
opt.length_id_ss = 5.6; %6.00; %7.00; %5.35;
opt.max_length   = 510;
opt.max_field    = 0.8; % 0.8;
opt.max_bendg    = 10.0;
opt.max_quadg    = 45.0;
...
% for i=1:length(bc)
%     opt.knobs.len_flag(bc(i)) = false;
%     opt.knobs.fld_flag(bc(i)) = false;
%     opt.knobs.grd_flag(bc(i)) = false;
% end
...
params = {
    0*5, (opt.naturalEmittance / (0.3e-9))^2; ...
    0*1, (opt.optics.max_betax / 35)^2; ...
    0*1, (opt.optics.max_betay / 35)^2; ...
    0*1, (opt.length_total / 480)^2; ...
    0*2, (opt.optics.etax(end) / 0.001)^2; ...
    };
        ...
        switch type
            case 1
                if ~isempty(opt.knobs.len_idx)
                    ...
                    if ~isnan(opt.knobs.len_max(elem)) && (nval > opt.knobs.len_max(elem)), continue; end;
                    if ~isnan(opt.knobs.len_min(elem)) && (nval < opt.knobs.len_min(elem)), continue; end;
            ...
            case 2
                if ~isempty(opt.knobs.fld_idx)
                    ...
                    nval = oval + step;
                    if ~isnan(opt.knobs.fld_max(elem)) && (nval > opt.knobs.fld_max(elem)), continue; end;
                    if ~isnan(opt.knobs.fld_min(elem)) && (nval < opt.knobs.fld_min(elem)), continue; end;
                    opt.the_ring{elem}.BendingAngle  = opt.the_ring{elem}.BendingAngle  * (nval/oval);
            ...
            case 3
                if ~isempty(opt.knobs.grd_idx)
                    ...
                    nval = oval + step;
                    if ~isnan(opt.knobs.grd_max(elem)) && (nval > opt.knobs.grd_max(elem)), continue; end;
                    if ~isnan(opt.knobs.grd_min(elem)) && (nval < opt.knobs.grd_min(elem)), continue; end;
                    K = nval / opt.b_rho;
...
% normaliza comprimentos de elementos permitidos, se necessário.
len = getcellstruct(opt.the_ring, 'Length', 1:length(opt.the_ring));
idx_trial = opt.knobs.len_idx;
idx_fixed = setdiff(1:length(opt.the_ring), idx_trial);
available = (opt.max_length / opt.nr_cells) - sum(len(idx_fixed));
if (sum(len(idx_trial)) > available)
    len(idx_trial) = len(idx_trial) * (available / sum(len(idx_trial)));
    opt.the_ring = setcellstruct(opt.the_ring, 'Length', 1:length(opt.the_ring), len);
end
opt.length_total = opt.nr_cells * sum(len);

% normaliza campos de elementos permitidos, se necessário.
bends = findcells(opt.the_ring, 'BendingAngle');
idx_trial = opt.knobs.fld_idx;
idx_fixed = setdiff(1:length(opt.the_ring), idx_trial);
ang0 = zeros(length(opt.the_ring),1);
ang0(bends) = getcellstruct(opt.the_ring, 'BendingAngle', bends);
ang = ang0;

iter = 0;
while true
    ang(idx_trial) = ((2*pi)/opt.nr_cells - sum(ang(idx_fixed))) * ang(idx_trial) / sum(ang(idx_trial));
    fld = opt.b_rho * (ang ./ len);
    fld(len == 0) = 0;
    sel = (abs(fld) > opt.max_field);
    pos = find(sel)';
    if isempty(pos), break; end;
    fld(sel) = opt.max_field;
    idx_fixed = [idx_fixed, pos];
    idx_trial = setdiff(idx_trial, pos);
    ang = fld .* len / opt.b_rho;
    iter = iter + 1;
    if iter > length(opt.the_ring), error_function; end
end

opt.the_ring = setcellstruct(opt.the_ring, 'BendingAngle', bends, ang(bends));
a = getcellstruct(opt.the_ring, 'EntranceAngle', bends); opt.the_ring = setcellstruct(opt.the_ring, 'EntranceAngle', bends, a .* (ang(bends)./ang0(bends)));
a = getcellstruct(opt.the_ring, 'ExitAngle', bends); opt.the_ring = setcellstruct(opt.the_ring, 'ExitAngle', bends, a .* (ang(bends)./ang0(bends)));

function error_function

    error('dunno what to do...help!\n');
