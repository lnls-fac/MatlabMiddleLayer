function r = analisa_dados_bba_do_diagmaq(varargin)
...
r.dir_name = uigetdir(r.dir_name, 'Selecione o diretório com as medidas de BBA do DiagMaq');
...
for i=1:length(files)
    ...
    file_name = fullfile(r.dir_name, files(i).name);
    ...
    counter = 1;
    while (counter <= length(r.bpms))
        if strcmpi(bpm_name, r.bpms{counter}), break; end
        counter = counter + 1;
    end
    if counter > length(r.bpms), continue; end
