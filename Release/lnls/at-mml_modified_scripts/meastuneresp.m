...
if ArchiveFlag
    if isempty(FileName)
        ...
        if isempty(DirectoryName)
            DirectoryName = [getfamilydata('Directory','DataRoot') 'Response', filesep, 'Tune', filesep];
        else
        ...
        [FileName, DirectoryName] = uiputfile('*.mat', 'Select a Tune Response File', [DirectoryName FileName]);
        if FileName == 0 
            ArchiveFlag = 0;
            disp('   Tune response measurement canceled.');
            Rmat = []; OutputFileName='';
            return
        end
